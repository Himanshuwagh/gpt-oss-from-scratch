...
# 1. RmsNorm implementation directly usig torch.nn.RMSNorm

# rms_norm = nn.RMSNorm([2, 3])
# input = torch.randn(2, 2, 3)
# rms_norm(input)

# 2. RmsNorm implementation from scratch

class RMSNorm(torch.nn.Module):
    def __init__(
            self, num_features, eps=1e-8, dtype=None, device=None):
        super().__init__()
        self.num_features = num_features
        self.eps = eps
        self.scale = torch.nn.Parameter(torch.ones(num_features, dtype=dtype, device=device))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        assert x.size(-1) == self.num_features, f"Expected input with last dimension {self.num_features}, but got {x.size(-1)}"
        t, dtype = x.float(), x.dtype
        t = t * torch.rsqrt(t.pow(2).mean(-1, keepdim=True) + self.eps)
        return (self.scale * t).to(dtype)
    

# RoPE (rotary postional encoding) Implementation
...
class RotaryEmbedding(torch.nn.Module):
    def __init__(
               ...
               base: int,
               dtype: torch.dtype,
               inital_context_length:int = 4096,
          ...
          self.base = base
          self.dtype = dtype
          self.initial_context_length = inital_context_length
          self.scaling_factor = scaling_factor
          self.ntk_alpha = ntk_alpha
          self.ntk_beta = ntk_beta
          self.device = device

    def _compute_concentratio_and_inv_freq(self) -> torch.tensor:
        freq = self.base ** (torch.arange(0, self.head_dim, 2, dtype=self.dtype) / self.head_dim)
    ...
    def _compute_cos_sin(
        ...
        concentration, inv_freq = self._compute_concentratio_and_inv_freq()
        t = (torch.arange(num_tokens, device=self.device, dtype=self.dtype))
...
def sdpa(Q, K, V, S, sm_scale, sliding_window=0):
    ...
    # K.shape = [3, 2, 2, 4]   # n_tokens, n_heads, q_mult, d_head
    # V.shape = [3, 2, 2, 4]
    K = K[:, :, None, :].expand(-1, -1, q_mult, -1)
    V = V[:, :, None, :].expand(-1, -1, q_mult, -1)

    # Original S.shape = [n_heads, q_mult, 1, 1]
    # After expand → [n_heads, q_mult, n_tokens, 1]
    # This is a bias term added to attention logits.
    S = S.reshape(n_heads, q_mult, 1, 1).expand(-1, -1, n_tokens, -1)

    # Shape: [n_tokens, n_tokens]
    # Upper-triangular -inf mask prevents future tokens from attending.
    mask = torch.triu(Q.new_full((n_tokens, n_tokens), -float("inf")), diagonal=1)
    if sliding_window > 0:
        mask = torch.minimum(mask, torch.triu(Q.new_full((n_tokens, n_tokens), -float("inf")), diagonal=-sliding_window))


    QK = torch.einsum("thqd,thkd->thqk", Q, K)  # [n_tokens, n_heads, q_mult, n_tokens]
    QK *= sm_scale
    QK += mask[None, None, :, :]  # broadcasting
    QK = torch.cat((QK, S), dim=-1)  # [n_tokens, n_heads, q_mult, n_tokens + 1]
    W = torch.softmax(QK, dim=-1)  # [n_tokens, n_heads, q_mult, n_tokens + 1]
    W = W[..., :-1]  # remove the extra column used for S
    attn = torch.einsum("hmqk,khmd->qhmd", W, V)  
    return attn.reshape(n_tokens, -1)  # [n_tokens, n_heads, q_mult, d_head]
